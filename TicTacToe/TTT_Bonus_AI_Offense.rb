...
class Board
  ...
  def imminent_win_square
    WINNING_LINES.each do |line|
      squares = @squares.values_at(*line)
      if two_identical_computer_markers(squares) && one_unmarked_square?(squares)
  ...
  def at_risk_square
    WINNING_LINES.each do |line|
      squares = @squares.values_at(*line)
      if two_identical_human_markers(squares) && one_unmarked_square?(squares)
        index = squares.find_index(squares.select(&:unmarked?).first)
        return line[index]
      end
    end
  ...
  def two_identical_human_markers(squares)
    ...
    markers.min == markers.max && markers.min == TTTGame::HUMAN_MARKER
  end
  
  def two_identical_computer_markers(squares)
    markers = squares.select(&:marked?).collect(&:marker)
    return false if markers.size != 2
     markers.min == markers.max && markers.min == TTTGame::COMPUTER_MARKER
  end
...
class TTTGame
  ...
  def computer_moves
    if board.imminent_win_square
      then board[board.imminent_win_square] = computer.marker
